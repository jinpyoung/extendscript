...
writeLog(logPath, "페이스북 자랑하기용 스토리카드 이미지 양산을 시작합니다.\n", "w")
arrayExtractionFromTxtFile(masterPath)  // CSV 파일에서 데이터 추출
main()
...
function main() {
    for (i = 0; dataArr.length > i; i++) {
        if (dataArr[i][2] == "ImageResKey") {
            ...
            var star_layers = starSet.layers // Star 그룹에 있는 레이어들
            var frameSet = doc.layerSets["Frame"]
            var frame_layers = frameSet.layers // Frame 그룹에 있는 레이어들
